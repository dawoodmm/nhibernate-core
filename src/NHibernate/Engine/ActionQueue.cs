...
namespace NHibernate.Engine
{
    ...
    public class ActionQueue
    {
        ...
        private readonly List<EntityUpdateAction> preDeleteUpdates;
        private readonly List<IExecutable> preDeleteInserts;
        ...
        private readonly List<CollectionPreDeleteUpdateAction> collectionPreDeleteUpdates;
        ...
        public ActionQueue(ISessionImplementor session)
        {
            ...
            preDeleteInserts = new List<IExecutable>(InitQueueListSize);
            deletions = new List<EntityDeleteAction>(InitQueueListSize);
            preDeleteUpdates = new List<EntityUpdateAction>(InitQueueListSize);
            ...
            collectionPreDeleteUpdates = new List<CollectionPreDeleteUpdateAction>(InitQueueListSize);
        ...
        public virtual void Clear()
        {
            ...
            insertions.Clear();
            deletions.Clear();
            preDeleteUpdates.Clear();
            preDeleteInserts.Clear();
            collectionCreations.Clear();
            collectionRemovals.Clear();
            collectionUpdates.Clear();
            collectionPreDeleteUpdates.Clear();
        }
        ...
        public void AddPreDeleteAction(EntityInsertAction action)
        {
            preDeleteInserts.Add(action);
        }
        ...
        public void AddAction(EntityDeleteAction action)
        {
            ...
            //var ReplaceInsert =deletions.SingleOrDefault(a=>a.)
            if (PreDeleteUpdate)
            {

                //((ISession)session).SaveOrUpdate(action);

                //this.SortActions();
                //this.SortCollectionActions();

                //preDeleteInserts.AddRange(insertions);
                //insertions.Clear();



                //preDeleteUpdates.AddRange(updates);
                //updates.Clear();


                //collectionPreDeleteUpdates.AddRange(collectionUpdates);
                //collectionUpdates.Clear();


                var dirtyInserts = new List<EntityAction>();

                this.find(action, dirtyInserts);


                if (dirtyInserts != null && dirtyInserts.Count > 0)
                {
                    for (int i = 0; i < dirtyInserts.Count; i++)
                    {
                        insertions.Remove(dirtyInserts[i]);
                        preDeleteInserts.Add(dirtyInserts[i]);
        ...
        private void find(EntityAction action, List<EntityAction> preDeleteInsertsdirty)
        {

            var EntityName = action.EntityName;
            var Id = action.Id;

            var inserts = insertions.Cast<EntityAction>();

            var preDeleteInsertAction = inserts.Any(i => i.EntityName == EntityName && i.Id == Id);

            if (preDeleteInsertAction)
            {
                preDeleteInsertsdirty.AddRange(inserts);
                insertions.Clear();
            }
            //if (preDeleteInsertAction != null)
            //{
            //    preDeleteInsertsdirty.Add(preDeleteInsertAction);
            //}
            return;
            foreach (var i in inserts)
            {
                bool any = false;
                var props = i.Persister.GetPropertyValues(i.Instance, session.EntityMode);
                for (int p = 0; p < props.Length && !any; p++)
                {
                    if (props[p] != null
                        && ((action.EntityName == this.session.BestGuessEntityName(props[p])
                             && action.Persister.EntityMetamodel.EntityType.IsEqual(action.Instance, props[p], this.session.EntityMode, this.session.Factory))
                            || i.Persister.IsDeleteDirty(props[p], action.Instance, this.session, p)))
                    {
                        any = true;
                    }
                ...
                if (any)
                {
                    preDeleteInsertsdirty.Add(i);
                    find(i, preDeleteInsertsdirty);
        ...
        private bool findPreDeleteInserts(EntityDeleteAction action, object[] props, int p, EntityAction i, bool any)
        {
            if (props[p] != null
                && ((action.EntityName == this.session.BestGuessEntityName(props[p]) && action.Persister.EntityMetamodel.EntityType.IsEqual(action.Instance, props[p], this.session.EntityMode, this.session.Factory))
                    || i.Persister.IsDeleteDirty(props[p], action.Instance, this.session, p)))
            {
                any = true;
            }
        ...
        {
            var updateEntityName = action.EntityName;
            var updateId = action.Id;
            var isInPreUpdateDeleteAction = (preDeleteUpdates.SingleOrDefault(i => i.EntityName == updateEntityName && i.Id == updateId)) != null;
            //if(isInPreUpdateDeleteAction)
               // return;
            

            if (true)
            {
                if (PreDeleteUpdate && (DeletionsCount > 0))
                {
                    var isInDeleteAction = deletions.SingleOrDefault(i => i.EntityName == updateEntityName && i.Id == updateId)!=null;
                    if (isInDeleteAction)
                    {
                        //if (!isInPreUpdateDeleteAction)
                        {
                            
                            preDeleteUpdates.Add(action);
                        }
                    }
                    else
                    {
        ...
        public void AddAction(CollectionUpdateAction action)
        {
            ...
            //if (PreDeleteUpdate && (DeletionsCount > 0))
            //{
            //    //var coll = action.Persister.CollectionType.GetElementsIterator(action.Collection);
            //    //foreach (var item in collectionUpdates)
            //    //{
            //    //    action.Persister.NodeName
            //    //    if (deletions.Any(d=>d)//(i => i.EntityName == updateEntityName && i.Id == updateId)))
            //    //}
            //    var isInPreUpdateDeleteAction = false;
            //    foreach (EntityDeleteAction d in this.deletions)
            //    {

            //        if (action.Persister.CollectionType.Contains(action.Collection.StoredSnapshot, d.Instance, this.session))
            //        {
            //            isInPreUpdateDeleteAction = true;
            //            break;
            //        }
            //    }
            ...
            //        //if (!isInPreUpdateDeleteAction)
            //        {
            //            var action2 = new CollectionPreDeleteUpdateAction(action.Collection, action.Persister, action.Key, action.emptySnapshot, session);

            //            collectionPreDeleteUpdates.Add(action2);
            //        }
            //    }
            //    else
            //    {
        ...
        public void AddPreDeleteAction(CollectionPreDeleteUpdateAction action)
        {
            collectionPreDeleteUpdates.Add(action);
        }
        public void AddAction(CollectionPreDeleteUpdateAction action)
        {
            collectionPreDeleteUpdates.Add(action);
        }
        ...
        public void AddPreDeleteAction(EntityUpdateAction preDeleteUpdateAction)
        {
            preDeleteUpdates.Add(preDeleteUpdateAction);
        }
        //public void AddAction(EntityPreDeleteInsertAction preDeleteInsertAction)
        //{
        //    preDeleteInserts.Add(preDeleteInsertAction);
        //}
        ...
        private static bool _preDeleteUpdate = true   ;
        public static bool PreDeleteUpdate
        {
            get { return _preDeleteUpdate; }
        }
        ...
        public void ExecuteActions()
        {
            ...
            {
                ExecuteActions(preDeleteInserts);
                ExecuteActions(preDeleteUpdates);
                ExecuteActions(collectionRemovals);
                ExecuteActions(collectionPreDeleteUpdates);
                ExecuteActions(deletions);
        ...
        public void PrepareActions()
        {
            ...
            PrepareActions(collectionPreDeleteUpdates);
        ...
        public virtual bool AreTablesToBeUpdated(ISet<string> tables)
        {
            return
                ...
                || AreTablesToUpdated(preDeleteInserts, tables)
                || AreTablesToUpdated(preDeleteUpdates, tables)
                || AreTablesToUpdated(insertions, tables)
                || AreTablesToUpdated(deletions, tables)
                || AreTablesToUpdated(collectionUpdates, tables)
                || AreTablesToUpdated(collectionPreDeleteUpdates, tables)
        ...
        public int CollectionPreDeleteUpdatesCount
        {
            get { return collectionPreDeleteUpdates.Count; }
        }
        ...
        public void SortCollectionActions()
        {
            if (session.Factory.Settings.IsOrderUpdatesEnabled)
            {
                ...
                collectionPreDeleteUpdates.Sort();
        ...
        private void SortInsertActions()
        {
            ...
            new PreDeleteInsertActionSorter(this).Sort();
        }
        ...
        public bool HasAnyQueuedActions
        {
            get
            {
                return
                    updates.Count > 0
                    || insertions.Count > 0
                    || preDeleteInserts.Count > 0
                    || preDeleteUpdates.Count > 0
                    || deletions.Count > 0
                    || collectionUpdates.Count > 0
                    || collectionPreDeleteUpdates.Count > 0
        ...
        public override string ToString()
        {
            ...
            return
                new StringBuilder().Append("ActionQueue[")
                                   .Append(" preDeleteInserts=")
                                   .Append(preDeleteInserts)
                                   .Append(" preDeleteUpdates=")
                                   .Append(preDeleteUpdates)
                                   .Append(" collectionPreDeleteUpdates=")
                                   .Append(collectionPreDeleteUpdates)
                                   .Append(" deletions=")
                                   .Append(deletions)
                                   .Append(" insertions=")
        ...
        [Serializable]
        private class PreDeleteInsertActionSorter
        {
            private readonly ActionQueue _actionQueue;
            ...
            public PreDeleteInsertActionSorter(ActionQueue actionQueue)
            {
                _actionQueue = actionQueue;

                //optimize the hash size to eliminate a rehash.
                _entityBatchNumber = new Dictionary<object, int>(actionQueue.preDeleteInserts.Count + 1);
            }

            public void Sort()
            {
                // the list of entity names that indicate the batch number
                foreach (EntityInsertAction action in _actionQueue.preDeleteInserts)
                {
                    // remove the current element from insertions. It will be added back later.
                    var entityName = action.EntityName;
                ...
                _actionQueue.preDeleteInserts.Clear();
                ...
                for (var i = 0; i < _actionBatches.Count; i++)
                {
                    ...
                    foreach (var action in batch)
                    {
                        _actionQueue.preDeleteInserts.Add(action);
                    }
                }
            }
