...
namespace NHibernate.Collection.Generic
{
    ...
    public class PersistentGenericList<T> : AbstractPersistentCollection, IList<T>, IList
    {
        ...
        public override PreEntityDeleteCollectionActionTypes PreEntityDeleteCollectionActionType
        {
            get
            {
                return PreEntityDeleteCollectionActionTypes.UpdateToNull;
            }
        }
        ...
        public override IEnumerable GetPreDeleteItems(ICollectionPersister persister, bool indexIsFormula)
        {
            ...
            var sn = (IList<T>)GetSnapshot();
            //var isColl = typeof(T).GetInterfaces().Any(i=>i is IEnumerable || (i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IEnumerable<>)));




            if (true)// (persister.ElementType.IsEntityType)
            {
            ...
            {
                deletes = sn.Except(WrappedList).ToList();
            }


          

            //var deletedItems2 = isColl ? sn.Where(c=>((IEnumerable)c).SequenceEqual()) sn.Except(WrappedList, ) : sn.Except(WrappedList).ToList();
            ...
            {
                return deletes.Select(sn.IndexOf);
            }
        }
        ...
        public override IEnumerable GetDeletes(ICollectionPersister persister, bool indexIsFormula)
        {
            ...
            if (ActionQueue.PreDeleteUpdate)
                sn = (IList<T>)this.StoredSnapshot;

            int end;
            //var deletedItems = sn.Where(snItem => !WrappedList.Contains(snItem));

            if (false)// (ActionQueue.PreDeleteUpdate)
            {

                var deletedItems = sn.Except(WrappedList).ToList();
                ...
                {
                    return deletedItems.Select(d => deletedItems.IndexOf(d));
                }
        ...
        public override bool NeedsInserting(object entry, int i, IType elemType)
        {
            ...
            if (ActionQueue.PreDeleteUpdate)
            {
                var ssn = (IList<T>)this.StoredSnapshot;
                return this.WrappedList[i] != null && (i >= ssn.Count || ssn[i] == null);
                //return WrappedList[i] != null && (i >= ssn.Count || ssn[i] == null);
            }
            else
            {
                return this.WrappedList[i] != null && (i >= sn.Count || sn[i] == null);
            }
        }

        public override bool NeedsUpdating(object entry, int i, IType elemType)
        {
            var sn = (IList<T>)((!ActionQueue.PreDeleteUpdate) ? GetSnapshot() : StoredSnapshot);
            return i < sn.Count && sn[i] != null && this.WrappedList[i] != null && elemType.IsDirty(this.WrappedList[i], sn[i], this.Session);
            if (ActionQueue.PreDeleteUpdate)
            {
                var dels = sn.Except(this.WrappedList).ToList();

                //if (i >= ssn.Count) return false;
               // return i < ssn.Count && ssn[i] != null && this.WrappedList[i] != null && !dels.Contains(ssn[i]) && elemType.IsDirty(this.WrappedList[i], ssn[i], this.Session);
                return i < sn.Count && sn[i] != null && this.WrappedList[i] != null && elemType.IsDirty(this.WrappedList[i], sn[i], this.Session);
            }
            else
            {
