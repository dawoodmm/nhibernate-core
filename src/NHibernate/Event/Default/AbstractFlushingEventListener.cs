...
namespace NHibernate.Event.Default
{
    ...
    public abstract class AbstractFlushingEventListener
    {
        ...
        private void MarkCollectionForPreDeleteUpdate(IPersistentCollection collection, CollectionEntry entry, IEventSource session)
        {
            
            var currentPersister = entry.LoadedPersister;
            if (collection.StoredSnapshot == null)
                return;
            var ss = currentPersister.CollectionType.GetElementsIterator(collection.StoredSnapshot);
            if (ss == null)
                return;
                
             var ss1 =   ss.Cast<object>();

            //var ss = (collection.StoredSnapshot as IEnumerable).Cast<object>();

            var dels = session.ActionQueue.CloneDeletions();
            if (ss1 == null || dels == null || !ss1.Any() || dels.Count == 0)
                return;

            var visitor = new FindDeleteDirtyItemsInElement(session, dels.Select(d => d.Instance));

            if (ss1.Select(e => (bool)visitor.ProcessValue(e, currentPersister.ElementType)).Any(deleted => deleted))
            {
                collection.PreDeleteDirty();
                entry.IsDoPreDeleteUpdate=true;
            }

            //var x = entries.Intersect(dels.Select(d => d.Instance)).Any();

        }

        private class FindDeleteDirtyItemsInElement : AbstractVisitor
        {
            private readonly IEnumerable<object> _deletedEntities;

            public FindDeleteDirtyItemsInElement(IEventSource session, IEnumerable deletedEntities)
                : base(session)
            {
                _deletedEntities = deletedEntities.Cast<object>();
            }

            internal override object ProcessEntity(object value, EntityType entityType)
            {
                return this._deletedEntities.Contains(value);

                //return base.ProcessEntity(value, entityType);
            }

            protected bool DeletedEntityReferenced { get; private set; }
        ...
        protected virtual void FlushCollections(IEventSource session)
        {
            ...
            foreach (DictionaryEntry me in list)
            {
                ...
                CollectionEntry ce = (CollectionEntry)me.Value;
                var persister = ce.CurrentPersister;
                if (session.ActionQueue.DeletionsCount > 0)
                {

                    //MarkCollectionForPreDeleteUpdate(coll, ce, session);

                //    DirtyCollectionSearchVisitor visitor = new DirtyCollectionSearchVisitor(session, persister.OwnerEntityPersister.PropertyVersionability);
                //    visitor.ProcessCollection(coll, )
                ...
                //    var ss = coll.StoredSnapshot;
                //    var entries = coll.Entries(ce.CurrentPersister); ;
                    
                //    var dels = session.ActionQueue.CloneDeletions();
                //    //var test = coll.EntryExists(session.ActionQueue.CloneDeletions()[1].Instance, 0);
                //    //if(ce)
                }
                ...
                if (ce.IsDoPreDeleteUpdate)
                {
                    session.Interceptor.OnCollectionUpdate(coll, ce.LoadedKey);
                    if (ActionQueue.PreDeleteUpdate)
                    {
                        var action = new CollectionPreDeleteUpdateAction(coll, ce.LoadedPersister, ce.LoadedKey, ce.IsSnapshotEmpty(coll), session);
                        //action.PreDeleteUpdate = true;
                        actionQueue.AddPreDeleteAction(action);
                    }
